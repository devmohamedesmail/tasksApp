...
export default function CarStatusItem({
  ...
  return (
    <Div flexDir="column" justifyContent="center" alignItems="center" mx={10}>
      <Image
        h={60}
        w={60}
        m={10}
      ...
      />
      <Text fontSize={17} fontWeight="bold">{name}</Text>
      <Div h={90}>
        {worker.map((i, w) => (<Text bg={Colors.primary} color="white" my={5} w='100%' px={15} py={5} rounded='lg' key={i} >{i}</Text>))}
      </Div>
      {status === "0" ? (
